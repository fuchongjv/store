...
class Bank:
    ...
    def bank_deposit(self, user):
        ...
        for d in data:
            if d[0] == user.getAccount():
                ...
                money = money + int(d[7])
                sql1 = "update user set money='" + str(money) + "' where account=" + user.getAccount()
                param = []
                update(sql1, param)
                return True
    ...
    # 银行的转账逻辑
    def bank_transfer(self, out_account, in_account):
        sql = "select * from user"
        param = [out_account.getAccount()]
        param2 = [in_account.getAccount()]
        data = select(sql, param)
        data2 = select(sql, param2)
        # 账号存在
        for c in data2:
            if c[0] == in_account.getAccount():
                for d in data:
                    if d[0] == out_account.getAccount():
                        password = input("请输入密码：")
                        if d[2] == password:
                            money = int(input("请输入金额："))
                            if money <= int(d[7]):
                                ...
                                sql1 = "update user set money='" + str(money1) + "' where account=" + out_account.getAccount()
                                sql2 = "update user set money='" + str(money2) + "' where account=" + in_account.getAccount()
                                update(sql1, [])
                                update(sql2, [])
                                return 0
                            else:
